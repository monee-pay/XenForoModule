...
namespace Monee\PaygateMonee\Payment;
...
class Monee extends AbstractProvider
{
	/**
	 * @return string
	 */
	...
	public function verifyConfig(array &$options, &$errors = []): bool
	{
		...
		{
			$errors[] = XF::phrase('mn_pg_mn_you_must_provide_all_data');
		}
	...
	public function initiatePayment(Controller $controller, PurchaseRequest $purchaseRequest, Purchase $purchase): XF\Mvc\Reply\AbstractReply
	{
		...
		if ($response)
		{
			...
			{
				if (strpos($responseData['message'], '"sum" is greather') !== false) {
					return $controller->error(XF::phrase('mn_pg_mn_sum_is_too_big'));
				}

				if (strpos($responseData['message'], '"sum" is less') !== false) {
					return $controller->error(XF::phrase('mn_pg_mn_sum_is_too_small'));
				}
			}
